...
def to_cmd(c, _path=None):
    command = f'PYTHONPATH=. python3 -m kbc.learn kbc/data/FB15k --model DistMult --max_epochs {c["e"]} --model_save_schedule 1000 --valid 3 --reg {c["n3"]} --batch_size {c["b"]} --rank {c["k"]}'
    return command


def to_logfile(c, path):
    outfile = "{}/fb15k_beaker_v1.{}.log".format(path, summary(c).replace("/", "_"))
    return outfile
...
def main(argv):
    ...
    path = 'logs/fb15k/fb15k_distmult_beaker_v1'
    is_rc = False
